...
Release-readiness boundary: these categories are API guidance for the current
pre-1.0 codebase, not a whole-crate production-ready claim. Stable / preferred
entries are stable only for their documented scope and do not override open
fidelity gaps, planned platform artifacts, or the release checklist. Any future
public release should re-run the API/release gates and independent review before
promoting broader production language.


## Current Release-Candidate Boundary

This map does not promote the whole crate to production-ready or 1.0-stable. It
is a scoped adoption guide: Stable / preferred entries are stable only for the
documented path and limitation set, while Beta, Compatibility-only,
Experimental, and Feature-gated entries retain their stated boundaries. Open or
demoted fidelity gaps, planned platform artifacts, and skipped release gates
continue to override any broad production or platform-support interpretation.
...
| --- | --- | --- |
| Responsive values | `egui_expressive::{Responsive, Breakpoints, BreakpointName}`, `egui_expressive::responsive::*` | Stable in `this API stability guide`; responsive tests and layout docs. |
...
These stable paths are still scoped by their docs. For example, data widgets are stable for read-only display, sort/filter/select fallback, property/tree presentation, and documented Forms v2 edit descriptors; advanced spreadsheet interactions and built-in large-table windowing remain unsupported in the current release boundary.

## Beta / Supported Public Paths

| Surface | Preferred public paths | Current support boundary |
| --- | --- | --- |
| Tailwind-style utility DSL | `egui_expressive::{Tw, ResponsiveTw, TwVariants, TwThemeVariants}` and `egui_expressive::tailwind::*` | Beta+. Rendered/recorded/bounded/unsupported behavior is defined in `docs/ui-framework/tw-render-contract.md`; deterministic egui-native layout/effect fixtures document the current behavior, but browser/CSS parity is still not claimed. |
| Drawing/effects | `egui_expressive::draw::*`, crate-root draw helpers such as `gradient_rect`, `zstack`, `with_opacity`, `clipped_shape` | Beta. Uses egui shapes and documented approximations; not a browser/CSS compositor. fixtures lock clipping/layered-background approximation behavior. |
...
| App-provided backdrop snapshots | `egui_expressive::{app_provided_backdrop_blur_report, app_provided_backdrop_blur_shape, BackdropCaptureRequest, BackdropCaptureSourceContract, BackdropCaptureSourceId, BackdropCaptureProviderId, BackdropCaptureSurfaceToken, BackdropCaptureFrameToken, BackdropCaptureConsent, BackdropFrameFreshness, BackdropOcclusionState, BackdropCapturePixelFormat, BackdropSnapshot, BackdropSnapshotProvider, install_backdrop_snapshot_provider, install_backdrop_snapshot_provider_with_source_contract, install_backdrop_capture_source_contract, load_backdrop_capture_source_contract, load_backdrop_snapshot_provider}` | Beta. Exactness is limited to app-provided tightly packed RGBA snapshots for one egui context/surface with an app-owned source contract that proves source/provider identity, surface/frame token, consent, current frame freshness, explicitly unoccluded source, DPI scale, physical size, and RGBA8 straight-alpha format. `BackdropCaptureRequest::with_expected_source_contract(...)` provides typed source/provider mismatch handling. Exact WGPU output still requires `wgpu`, `init_gpu_effects_for_context(...)`, installed provider/source contract, `radius >= 1.0`, and in-budget dimensions. It is not host/native capture, browser `backdrop-filter`, or Tailwind `backdrop_blur` parity. |
| WGPU app-owned backdrop source contract | `egui_expressive::{AppOwnedBackdropSurfaceId, AppOwnedBackdropFrameId, AppOwnedBackdropAlphaMode, AppOwnedOffscreenBackdropSource, install_app_owned_offscreen_backdrop_source, load_app_owned_offscreen_backdrop_source, bind_app_owned_offscreen_backdrop_source_for_context, app_owned_offscreen_backdrop_blur_report, app_owned_offscreen_backdrop_blur_shape}` with `wgpu` | Experimental in fidelity item B3/B4. Host apps may register an app-owned same-frame offscreen `TextureView` source for one egui context/surface and bind it to the active `egui-wgpu` renderer before exact report/shape helpers can sample it. Exactness also requires the current installed source to be the same source allocation retained by the sidecar. Missing/unbound/stale/wrong metadata, same-metadata source reinstall without rebinding, and absent WebGPU support fail non-exact. B4 documents this as an app-owned WGPU support contract only: native/host capture, browser `backdrop-filter`, and default Tailwind backdrop blur remain out of scope until separate source contracts are approved. |
| Native backdrop adapter substrate | `egui_expressive::{NativeBackdropInitError, NativeBackdropPlatform, NativeBackdropSupportFamily, NativeBackdropSupportState, NativeBackdropPermissionState, NativeBackdropSourceScope, NativeBackdropContractDiagnostic, NativeBackdropSmokeArtifact}` with `native-backdrop` | Experimental / feature-gated. The native-backdrop substrate freezes feature names, platform-family labels, support/permission/source-scope states, redacted diagnostics, and manual-smoke artifact vocabulary only. It does not capture native pixels, does not enable `HostFramebufferBackdrop`, and does not imply any platform provider is implemented. |
...
| Style/theme/tokens/icons/typography | `egui_expressive::{DesignTokens, Theme, SemanticColors, Icon, TypeScale, TypeSpec, FontRegistry, FontFaceRecord, FontFamilyRecord, FontSelectionReport}`, `style`, `theme`, `icons`, `typography` modules | Beta. Token and typography docs exist; Phase 4B reduced `typography/mod.rs` to a facade while deeper typography layout/render files remain justified exceptions. Font registry APIs are opt-in app-provided metadata/report helpers; they do not bundle assets, enumerate OS fonts, or stabilize final raster/codegen parity. |
...
| Scene/render model | `egui_expressive::scene::*`, crate-root `SceneNode`, `ArtboardScene`, `render_scene` | Beta. Scene model is public and tested; visual fidelity follows Tailwind/draw contract boundaries. scene-effect and gradient-mesh diagnostics are report-backed and bounded; exactness claims still follow `docs/ui-framework/render-fidelity-contract.md`. |
| Render fidelity reports and capabilities | `egui_expressive::{RenderCapabilities, RenderReport, RenderIssueKind, RenderQuality, RenderBackendKind, RenderFeature, OffscreenRequest}` | Beta / bounded contract. These public types let apps inspect backend capability, requested/actual quality, and WGPU lifecycle degradation, but they do not by themselves prove production-fidelity output. WGPU exactness claims still require the feature-gated WGPU APIs below plus runtime/context readiness and issue-free reports. |
...
| `PianoRoll` alias | `PianoRollView` | Compatibility alias for the view-only rename decision. |
...
| `egui_expressive::gpu::*` | WGPU production-fidelity bridge behind `wgpu`; platform/device behavior and dependency policy are not default API guarantees. Non-WGPU default builds are compatibility builds and cannot close WGPU production-fidelity evidence. |
...
| --- | --- | --- |
| `wgpu` / `gpu-effects` | `egui_expressive::gpu::{init_gpu_effects, init_gpu_effects_for_context, bind_app_owned_offscreen_backdrop_source_for_context, GpuEffectsResources, GpuCompositeCallback, GpuSourceLayerEffectCallback, GpuEffectSource, WgpuLifecycleFailure, wgpu_lifecycle_report, wgpu_source_layer_effect_report, wgpu_app_provided_backdrop_snapshot_report, host_framebuffer_backdrop_report}` | WGPU production-fidelity tier. The feature remains opt-in so default builds stay lightweight compatibility builds, but exact production-fidelity render/effect claims require this feature path plus successful WGPU runtime/resource/context initialization. Source-layer exact-effect claims are bounded to initialized callback resources and caller/library-owned solid rectangular RGBA sources using the Phase 9A/9B two-pass blur callback for blur and padded shadow subsets. App-provided backdrop exactness is only exposed through `wgpu_app_provided_backdrop_snapshot_report(...)`, which takes a validated `BackdropCaptureSourceContract`; the generic `wgpu_source_layer_effect_report(...)` keeps `GpuEffectSource::AppProvidedBackdropSnapshot` non-exact because it has no source-contract parameter. fidelity item B3 permits app-owned `TextureView` backdrop sampling only through the renderer-bound binding API plus app-owned report/shape helpers; direct generic `GpuEffectSource::AppOwnedOffscreenBackdrop` reports remain non-exact without sidecar proof. Scene auto-selection requires `init_gpu_effects_for_context` so readiness is scoped to an egui context rather than process-global. `WgpuLifecycleFailure` / `wgpu_lifecycle_report` expose deterministic report vocabulary for missing runtime, missing context binding, unsupported adapter/device, device loss, pipeline creation failure, resource allocation failure, and WGPU budget failures. `host_framebuffer_backdrop_report` is a fail-closed unsupported report helper, not a capture implementation. These APIs must not imply host framebuffer capture, non-rect scene effects, zero/sub-pixel shadow exactness, browser `backdrop-filter`, or broad CSS backdrop/drop-shadow parity. |
| `native-backdrop` | `egui_expressive::{NativeBackdropInitError, NativeBackdropPlatform, NativeBackdropSupportFamily, NativeBackdropSupportState, NativeBackdropPermissionState, NativeBackdropSourceScope, NativeBackdropContractDiagnostic, NativeBackdropSmokeArtifact, NATIVE_BACKDROP_*_FEATURE}` | Common native-adapter contract substrate only. Platform-specific providers require later child blockers and target features such as `native-backdrop-x11`; no native capture feature is default-enabled. |
...
- New public modules start as Experimental unless the adding stage also ships docs, tests/examples, limitations, and independent review for Beta or Stable.
...
API/release status should be validated with:
...
The API-label audit keeps
release readiness blocked until a future approved release plan supplies full
validation, platform/Web artifacts for claimed targets, closed or accepted fidelity backlog
blockers, and final clean independent review.
