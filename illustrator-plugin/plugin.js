...
function hasTextFontCodegenOverride(style) {
  if (!style) return false;
  const weight = Number(style.weight ?? style.fontWeight ?? style.font_weight);
  const hasNonDefaultWeight = Number.isFinite(weight) && Math.abs(weight - 400) > 0.0001;
  const family = style.family ?? style.fontFamily ?? style.font_family;
  const hasExplicitFamily = family !== undefined && family !== null && String(family).trim().length > 0;
  return hasNonDefaultWeight || hasExplicitFamily;
}

function hasElementFontCodegenOverride(el) {
  if (!el) return false;
  if (hasTextFontCodegenOverride(el) || hasTextFontCodegenOverride(el.textStyle)) return true;
  return Array.isArray(el.textRuns) && el.textRuns.some(run => hasTextFontCodegenOverride(run && run.style));
}
...
function shouldEmitParitySidecar(options) {
  return isStrictCodeOnly(options) || !!(options?.sidecar || options?.includeSidecar);
}
...
function parityFindingsForElement(el, options) {
  ...
  if (shapingReasons.length > 0) {
    ...
    if (hasElementFontCodegenOverride(el)) {
      const fontStatus = isStrictCodeOnly(options) ? "unsupported" : "approximate";
      add(fontStatus, "font weight/family codegen requires outlined glyphs or an approved app-provided font registry/provenance contract");
    }
...
function aiParserIntegrityManifestPath(pluginDir) {
    const path = getNodeModule("path") || { join: (...args) => args.join("/").replace(/\/+/g, "/") };
    return path.join(pluginDir, "ai-parser-integrity.json");
}

function aiParserRelativePath(pluginDir, binaryPath) {
    const path = getNodeModule("path");
    if (!path || typeof path.relative !== "function") return null;
    const relative = path.relative(pluginDir, binaryPath).replace(/\\/g, "/");
    if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) return null;
    return relative;
}

function expectedAiParserSha256(manifest, relativePath) {
    const direct = manifest && manifest[relativePath];
    if (typeof direct === "string") return direct.toLowerCase();
    const nested = manifest && manifest.binaries && manifest.binaries[relativePath];
    if (typeof nested === "string") return nested.toLowerCase();
    return null;
}

function verifyAiParserBinaryIntegrity(pluginDir, binaryPath) {
    const fs = getNodeModule("fs");
    const crypto = getNodeModule("crypto");
    if (!fs || typeof fs.readFileSync !== "function" || !crypto || typeof crypto.createHash !== "function") {
        noteAiParserDiagnostic("Cannot verify bundled ai-parser integrity", "Node fs/crypto APIs unavailable in this host");
        return false;
    }
    const relative = aiParserRelativePath(pluginDir, binaryPath);
    if (!relative) {
        noteAiParserDiagnostic("Refusing bundled ai-parser", "candidate path is outside the extension bundle");
        return false;
    }
    const manifestPath = aiParserIntegrityManifestPath(pluginDir);
    let manifest;
    try {
        manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
    } catch (e) {
        noteAiParserDiagnostic("Refusing bundled ai-parser", "integrity manifest ai-parser-integrity.json is missing or invalid");
        return false;
    }
    const expected = expectedAiParserSha256(manifest, relative);
    if (!expected || !/^[a-f0-9]{64}$/.test(expected)) {
        noteAiParserDiagnostic("Refusing bundled ai-parser", `integrity manifest does not contain a sha256 for ${relative}`);
        return false;
    }
    let actual;
    try {
        actual = crypto.createHash("sha256").update(fs.readFileSync(binaryPath)).digest("hex");
    } catch (e) {
        noteAiParserDiagnostic("Refusing bundled ai-parser", "could not hash candidate binary");
        return false;
    }
    if (actual !== expected) {
        noteAiParserDiagnostic("Refusing bundled ai-parser", `sha256 mismatch for ${relative}`);
        return false;
    }
    return true;
}
    ...
    for (const candidate of candidates) {
        try {
            if (fs.existsSync(candidate)) {
                if (!verifyAiParserBinaryIntegrity(pluginDir, candidate)) {
                    aiParserStatus.available = false;
...
async function exportArtboards(selectedIndices, options, selectedTiles) {
  ...
  for (const r of results) {
    ...
    if (shouldEmitParitySidecar(options)) files[`${sn}.json`] = generateSidecar(r.artboard, r.elements, colorMap, options);
  }
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    aiParserIntegrityManifestPath,
    verifyAiParserBinaryIntegrity,
    mergeAiParserData,
    collectWarnings,
