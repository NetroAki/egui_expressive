...
function generateArtboardFile(ab, els, colorMap, stateName, comps) {
  ...
  let usesRichText = false, usesTypeSpec = false, usesShadow = false, usesBlur = false, usesComponents = comps && comps.length > 0;
  const walk = (elements) => {
    for (const el of elements) {
      if (el.type === "text") usesRichText = true;
      if (el.type === "group" && el.children?.length > 0) usesTypeSpec = true;
  ...
  let imports = ["Color32", "Ui", "Vec2", "Rect"];
  if (usesRichText) imports.push("RichText");
  
  let exprImports = ["with_alpha"];
  if (usesTypeSpec) { exprImports.push("vstack", "hstack", "TypeSpec"); }
