...
@group(2) @binding(0) var<uniform> blur: AppOwnedBackdropBlurUniforms;
...
fn fs_main(in: VertexOut) -> @location(0) vec4<f32> {
    ...
    let uv_origin = vec2<f32>(blur.uv_origin_x, blur.uv_origin_y);
    let uv_scale = vec2<f32>(blur.uv_scale_x, blur.uv_scale_y);
    let base_uv = uv_origin + in.uv * uv_scale;
    let offsets = array<f32, 5>(-1.0, -0.5, 0.0, 0.5, 1.0);
    let weights = array<f32, 5>(0.06136, 0.24477, 0.38774, 0.24477, 0.06136);

    var color = vec4<f32>(0.0);
    for (var i = 0u; i < 5u; i = i + 1u) {
        color = color + textureSample(source_texture, source_sampler, base_uv + direction * offsets[i] * radius) * weights[i];
    }
    return color;
}
