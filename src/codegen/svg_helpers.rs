...
pub(crate) fn parse_top_level_elements(svg: &str) -> Vec<LayoutElement> {
...
pub(crate) fn find_matching_close(s: &str, tag: &str) -> Option<usize> {
    let open = format!("<{}", tag);
...
pub(crate) fn extract_float_attr(tag: &str, attr: &str) -> Option<f32> {
    extract_attr(tag, attr)?.parse().ok()
}

pub(crate) fn extract_fill_from_tag(tag: &str) -> Option<Color32> {
    // Try fill attribute
...
pub(crate) fn extract_stroke_from_tag(tag: &str) -> Option<(f32, Color32)> {
...
pub(crate) fn extract_transform_xy(tag: &str) -> (Option<f32>, Option<f32>) {
...
pub(crate) fn estimate_path_bounds(d: &str) -> (f32, f32) {
